...
const resolveVonagePrivateKey = () => {
  ...
  if (
    ...
    envKey.includes("END PRIVATE KEY")
  ) {
    return envKey.replace(/\\n/g, "\n");
  }

  const envPath = process.env.VONAGE_PRIVATE_KEY_PATH;
  if (envPath && fs.existsSync(envPath)) {
    return fs.readFileSync(envPath, "utf8");
  }

  const defaultPath = path.join(process.cwd(), "lib", "private.key");
...
export async function bookAppointment(formData) {
  ...
  try {
    // Get the user and ensure they are a patient
